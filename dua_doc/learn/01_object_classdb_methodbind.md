...
C++ 本身是个"编译完就忘"的语言——编译完之后，代码里的类名、方法名全都变成了内存地址，运行时你拿着一个字符串 `"set_position"` 去找对应的函数？C++ 表示"我不认识"。

但 Godot 引擎偏偏到处都需要这个能力：
- 编辑器的 Inspector 要列出"这个节点有哪些属性"
- GDScript 写 `node.set_position(...)` 要能调到 C++ 的函数
- 保存场景时要能把对象的所有属性按名字存下来

所以 Godot 自己搞了一套**"花名册"系统**。

### 三个主角各干啥？

**Object**——所有东西的祖宗。每个 Object 一出生就去"派出所"（ObjectDB）上户口，拿到一个 64 位的身份证号（ObjectID）。以后别人想找你，不用记你的内存地址（怕你搬家后变成悬垂指针），拿身份证号去派出所查就行，查不到说明你已经"不在了"。

**ClassDB**——花名册本体。一个巨大的全局 HashMap，key 是类名，value 是这个类的全部信息：有哪些方法、有哪些属性、setter/getter 是什么、能不能被实例化……整个引擎只有这一份，谁都可以来查。

**MethodBind**——方法的"翻译官"。C++ 的成员函数签名五花八门（有 const 的、有返回值的、有可变参数的……），但对外只暴露三个统一接口：
- `call`：传 Variant 进来，我帮你拆箱、类型检查、调用——给 GDScript 这种动态语言用
- `validated_call`：你已经确认类型没问题了，我跳过检查直接调——GDScript 编译器优化后用
- `ptrcall`：直接传原始指针，零封装零检查——给 C#、GDExtension 这种原生绑定用

### GDCLASS 宏干了啥？

你在每个类里写的 `GDCLASS(MyNode, Node)` 展开后大约 100 行，干了这些事：

1. **给自己贴标签**：生成一个静态唯一地址（`get_class_ptr_static`），用来做类型判断
2. **自动分发 _notification**：生成代码保证继承链上每一层的 `_notification` 都会被调用，你不用写 `super()`
3. **注册到 ClassDB**：`initialize_class()` 会先确保父类注册了，然后注册自己，最后调用 `_bind_methods()` 把方法/属性/信号全部录入花名册

其中有个巧妙的技巧：**比较函数指针**。如果子类没有覆写 `_notification`，那子类的函数指针和父类的一样，引擎就跳过不调，省了一次空调用。

### 一次方法调用发生了什么？

GDScript 里写 `node.set_position(Vector2(10, 20))`，实际发生的事：

```
"set_position" 这个字符串
    → Object::callp() 接到了
    → 先问脚本实例："你有这个方法吗？" → "没有"
    → 再去 ClassDB 查花名册 → 找到了 MethodBind
    → MethodBind.call()
        → 从 Variant 里把 Vector2(10,20) 拆出来
        → 调用真正的 C++ 函数 node->set_position(Vector2(10,20))
```

**脚本优先**——如果你在 GDScript 里定义了同名方法，会先走脚本的，C++ 的不会被调到。这就是脚本"覆写"引擎方法的原理。

### 类型判断为什么快？

判断"这个对象是不是 Node？"有三档速度：

1. **编译期就知道**：如果代码里写死了类型，编译器直接优化掉，零开销
2. **一条位运算指令**：Godot 给 15 个最常用的基类（Node、Resource、Control……）各分了一个 bit，判断时做一次 AND 就行
3. **遍历继承链**：其他不在"VIP 名单"里的类，才需要沿继承链逐个比较指针

### 一句话总结

> **Godot 用一个宏（GDCLASS）让每个 C++ 类在编译时自动生成"自我介绍"，启动时统一录入全局花名册（ClassDB），运行时任何人都能通过字符串名字查到并调用任何类的方法/属性/信号。** 这就是整个反射系统干的事。
